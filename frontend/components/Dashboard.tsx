...
function ParetoChart({ title, rows, labelKey, displayUnit }: { title: string; rows: any[]; labelKey: "reason" | "department"; displayUnit: "kg" | "tons" }) {
  ...
  let running = 0;
  const data = rows.map(row => { running += Number(row.net || 0); return { name: String(row[labelKey] || "Other"), value: Number(row.net || 0) / (displayUnit === "tons" ? 1000 : 1), cumulative: total ? (running / total) * 100 : 0 }; });
...
function CombinedRejectionChart({ rows, displayUnit }: { rows: any[]; displayUnit: "kg" | "tons" }) {
  ...
  const data = rows.map(row => ({ name: row.reason, ...Object.fromEntries(departments.map(dept => [dept, Number(row.departments?.[dept] || 0) / (displayUnit === "tons" ? 1000 : 1)])) }));
