...
export default function Dashboard({ onUploadNew }: { onUploadNew?: () => void }) {
  ...
  const [reportRejection, setReportRejection] = useState<any>(null);
  const [lineWasteReport, setLineWasteReport] = useState<any>(null);
  ...
  useEffect(() => { if (!hasLoadedData) return; fetch(`${API}/api/kpis/rejection-reasons?_=${Date.now()}`, { cache: "no-store" }).then(response => response.ok ? response.json() : Promise.reject(new Error("Could not load rejection Pareto data."))).then(setReportRejection).catch(() => setReportRejection(null)); }, [hasLoadedData, reportingPeriod]);
  useEffect(() => { if (!hasLoadedData) return; fetch(`${API}/api/kpis/line-waste?_=${Date.now()}`, { cache: "no-store" }).then(response => response.ok ? response.json() : Promise.reject(new Error("Could not load downtime Pareto data."))).then(setLineWasteReport).catch(() => setLineWasteReport(null)); }, [hasLoadedData, reportingPeriod]);

  const chart = useMemo(() => {
    const ranked = (data?.rows || []).map(row => ({ name: String(row.Film || "Unspecified").slice(0, 24), value: Number(row["Overall CPP waste"] || 0) })).filter(item => item.value > 0).sort((a, b) => b.value - a.value).slice(0, 8);
  ...
  return <main data-film-filtered={filmFilter ? "true" : "false"} className="waste-shell min-h-screen text-slate-900">
    ...
    <FilmWiseReport chart={chart} columns={columns} data={data} reportRejection={reportRejection} lineWasteReport={lineWasteReport} displayUnit={displayUnit} />
    <ReportTableAccordion columns={columns} data={data} />
...
function ParetoChart({ title, rows, labelKey, displayUnit }: { title: string; rows: any[]; labelKey: "reason" | "department"; displayUnit: "kg" | "tons" }) {
  const total = rows.reduce((sum, row) => sum + Number(row.net || 0), 0);
  let running = 0;
  const data = rows.map(row => { running += Number(row.net || 0); return { name: String(row[labelKey] || "Other"), value: Number(row.net || 0), cumulative: total ? (running / total) * 100 : 0 }; });
  return <section className="rounded-xl border border-slate-200 p-3"><h3 className="mb-2 text-sm font-bold text-slate-800">{title}</h3><div className="h-56"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 34 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0"/><XAxis dataKey="name" angle={-30} textAnchor="end" interval={0} height={58} tick={{ fontSize: 10 }}/><YAxis yAxisId="weight" tick={{ fontSize: 10 }}/><YAxis yAxisId="percent" orientation="right" domain={[0, 100]} tickFormatter={value => `${value}%`} tick={{ fontSize: 10 }}/><Tooltip formatter={(value: number, name: string) => name === "cumulative" ? [`${value.toFixed(1)}%`, "Cumulative"] : [`${format(value, "", displayUnit)} ${displayUnit === "tons" ? "tons" : "kg"}`, "Net Wt"]}/><Bar yAxisId="weight" dataKey="value" fill="#0f766e" radius={[4, 4, 0, 0]}/><Line yAxisId="percent" type="monotone" dataKey="cumulative" stroke="#e11d48" strokeWidth={2} dot={{ r: 2 }}/></ComposedChart></ResponsiveContainer></div></section>;
}

function CombinedRejectionChart({ rows, displayUnit }: { rows: any[]; displayUnit: "kg" | "tons" }) {
  const departments = Array.from(new Set(rows.flatMap(row => Object.keys(row.departments || {}))));
  const data = rows.map(row => ({ name: row.reason, ...Object.fromEntries(departments.map(dept => [dept, Number(row.departments?.[dept] || 0)])) }));
  const colors = ["#0f766e", "#2563eb", "#e11d48", "#d97706", "#7c3aed"];
  return <section className="rounded-xl border border-slate-200 p-3"><h3 className="mb-2 text-sm font-bold text-slate-800">Rejection by Reason and Responsible Department</h3><div className="h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 48 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0"/><XAxis dataKey="name" angle={-30} textAnchor="end" interval={0} height={70} tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }}/><Tooltip formatter={(value: number, name: string) => [`${format(value, "", displayUnit)} ${displayUnit === "tons" ? "tons" : "kg"}`, name]}/>{departments.map((dept, index) => <Bar key={dept} dataKey={dept} stackId="rejection" fill={colors[index % colors.length]} />)}</BarChart></ResponsiveContainer></div></section>;
}

function formatWeightTooltip(valueKg: number, displayUnit: "kg" | "tons") {
  const shown = displayUnit === "tons" ? valueKg / 1000 : valueKg;
...
function DowntimeParetoChart({ rows, filmCategories }: { rows: any[]; filmCategories?: any[] }) {
  ...
  return <section className="rounded-xl border border-slate-200 p-3"><h3 className="mb-2 text-sm font-bold text-slate-800">Line Waste Downtime by Category</h3><div className="h-72"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 48 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0"/><XAxis dataKey="name" angle={-30} textAnchor="end" interval={0} height={70} tick={{ fontSize: 10 }}/><YAxis yAxisId="hours" label={{ value: "Hours", angle: -90, position: "insideLeft" }} tick={{ fontSize: 10 }}/><YAxis yAxisId="percent" orientation="right" domain={[0, 100]} tickFormatter={value => `${value}%`} tick={{ fontSize: 10 }}/><Tooltip formatter={(value: number, name: string) => name === "cumulative" ? [`${Number(value).toFixed(2)}%`, "Cumulative"] : [`${Number(value).toFixed(1)}`, "Hours"]}/><Bar yAxisId="hours" dataKey="hours" fill="#0f766e" radius={[4, 4, 0, 0]}/><Line yAxisId="percent" type="monotone" dataKey="cumulative" stroke="#f59e0b" strokeWidth={3} dot={{ r: 3, stroke: "#fff", strokeWidth: 2 }}/></ComposedChart></ResponsiveContainer></div></section>;
}

function ReportTableAccordion({ columns, data }: { columns: string[]; data: Data | null }) {
  return <details className="report-table-accordion panel overflow-hidden"><summary className="flex cursor-pointer list-none items-center justify-between px-5 py-5"><span><span className="section-kicker block">Report details</span><span className="mt-1 block font-bold">Waste by film</span></span><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600">{data?.total ?? 0} records ▾</span></summary><div className="max-h-[410px] overflow-auto"><table className="min-w-full text-left text-sm"><thead className="sticky top-0 z-10 bg-slate-50 text-[10px] uppercase tracking-wider text-slate-500"><tr>{columns.map(column => <th key={column} className="whitespace-nowrap px-4 py-3.5 font-bold">{column}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{data?.rows.map((row, index) => <tr key={index}>{columns.map(column => <td key={column} className="whitespace-nowrap px-4 py-3.5">{format(row[column], column.includes("%") ? "%" : "")}{column.includes("%") && row[column] != null ? "%" : ""}</td>)}</tr>)}</tbody></table></div></details>;
}

function FilmWiseReport({ chart, columns, data, reportRejection, lineWasteReport, displayUnit }: { chart: any[]; columns: string[]; data: Data | null; reportRejection: any; lineWasteReport: any; displayUnit: "kg" | "tons" }) {
  const [reportOpen, setReportOpen] = useState(false);
  const displayChart = chart.map(item => ({ ...item, value: Number((item.value / (displayUnit === "tons" ? 1000 : 1)).toFixed(displayUnit === "tons" ? 3 : 2)) }));
  return <div className="film-wise-report mt-7 space-y-6">
    <section className="panel film-wise-chart p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="section-kicker">Priority analysis</p><h3 className="mt-1 font-bold">Waste Pareto by Film</h3></div><span className="rounded-lg bg-teal-50 px-2.5 py-1 text-xs font-bold text-teal-700">kg / %</span></div><div className="mt-5 h-80"><ResponsiveContainer><ComposedChart data={displayChart} margin={{ top: 8, right: 16, left: 0, bottom: 48 }}><CartesianGrid stroke="#e2e8f0" strokeDasharray="3 3"/><XAxis dataKey="name" angle={-28} textAnchor="end" height={68} tick={{ fontSize: 10 }}/><YAxis yAxisId="waste" tick={{ fontSize: 10 }}/><YAxis yAxisId="share" orientation="right" domain={[0, 100]} tickFormatter={value => `${value}%`} tick={{ fontSize: 10 }}/><Tooltip formatter={(value: number, name: string) => name === "cumulative" ? [`${Number(value).toFixed(2)}%`, "Cumulative"] : [formatWeightTooltip(Number(value) * (displayUnit === "tons" ? 1000 : 1), displayUnit), "Overall CPP Waste"]}/><Bar yAxisId="waste" dataKey="value" fill="#0f766e" radius={[6, 6, 0, 0]} barSize={25}/><Line yAxisId="share" type="monotone" dataKey="cumulative" stroke="#f59e0b" strokeWidth={3} dot={{ r: 3 }}/></ComposedChart></ResponsiveContainer></div></section>
    {reportRejection && <ParetoChart title="Rejection by Reason" rows={reportRejection.reasons || []} labelKey="reason" displayUnit={displayUnit}/>} 
    {reportRejection && <ParetoChart title="Rejection by Responsible Department" rows={reportRejection.departments || []} labelKey="department" displayUnit={displayUnit}/>} 
...
function RejectionDebugModal({ summary, error, displayUnit, onClose }: { summary: any; error: string; displayUnit: "kg" | "tons"; onClose: () => void }) {
  const value = (amount: number) => `${format(amount, "", displayUnit)} ${displayUnit === "tons" ? "tons" : "kg"}`;
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4"><section role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-3xl bg-white shadow-2xl"><header className="flex items-start justify-between bg-gradient-to-br from-teal-800 to-cyan-700 p-6 text-white"><div><p className="text-xs font-bold uppercase tracking-wider text-teal-100">Rejection detail</p><h2 className="mt-2 text-xl font-bold">Rejection by reason</h2><p className="mt-1 text-2xl font-bold">{summary ? value(summary.total.net) : "Loading..."}</p></div><button onClick={onClose} className="text-2xl">×</button></header><div className="space-y-5 p-5">{error ? <p className="text-rose-600">{error}</p> : summary && <><ParetoChart title="Rejection by Reason" rows={summary.reasons} labelKey="reason" displayUnit={displayUnit}/><ParetoChart title="Rejection by Responsible Department" rows={summary.departments || []} labelKey="department" displayUnit={displayUnit}/><CombinedRejectionChart rows={summary.combined || []} displayUnit={displayUnit}/>{summary.reasons.map((row: any) => <details key={row.reason} className="rounded-xl border border-slate-200"><summary className="flex cursor-pointer list-none justify-between px-4 py-3 font-semibold"><span>{row.reason} <span className="text-xs font-normal text-slate-500">({row.count} rows)</span></span><span>{value(row.net)}</span></summary><div className="overflow-auto border-t border-slate-100 px-4 py-2"><table className="min-w-full text-xs"><thead className="text-left text-slate-500"><tr><th className="py-2 pr-4">Raw Reason</th><th className="py-2 pr-4 text-right">Net Wt</th><th className="py-2 pr-4">Month</th><th className="py-2">Responsible Dept</th></tr></thead><tbody>{(row.detail || []).map((item: any, index: number) => <tr key={`${row.reason}-${index}`} className="border-t border-slate-100"><td className="py-2 pr-4">{item.raw_reason}</td><td className="py-2 pr-4 text-right">{value(item.net)}</td><td className="py-2 pr-4">{item.month_period || item.month || "—"}</td><td className="py-2">{item.responsible_dept || "Other"}</td></tr>)}</tbody></table></div></details>)}</>}</div></section></div>;
}
